...
service_name = github_repository.split("/")[1]
...
def npm_deploy(directory):
    ...
    subprocess.run(args=["npm", "install"], cwd=directory, stdout=sys.stdout, stderr=sys.stderr)
    subprocess.run(args=["npm", "run", "build"], cwd=directory, stdout=sys.stdout, stderr=sys.stderr)
    subprocess.run(args=["npm", "publish"], cwd=directory, stdout=sys.stdout, stderr=sys.stderr)
...
def build_typescript_fetch():
    run_generator(
        ...
        additional_properties=gen_additional_properties({
            "npmName": f"@{github_owner}/{service_name}",
    ...
    npm_deploy("/openapi-generator/" + service_name)
